...
class Dongle extends EventEmitter {
  ...
  configure( options ) {
    return this.command( ADDR_DONGLE, OP_CONFIGURE, Buffer.from([  ]) );
  }

  // Returns a promise that resolves when the state of the keyswitch is set
  keyswitch( state ) {

    return this.command( ADDR_DONGLE, OP_KEYSWITCH, Buffer.from([ (state)? 1:0 ]) );
  }
  ...
  command( dest, id, values, options ) {
    ...
    return new Promise( function( resolve, reject ) {
      ...
      options.onComplete = function(err, response) { console.log( response );
  ...
  watch( slot, address, length, cb ) {
    ...
    return this.command( ADDR_DONGLE, OP_WATCH, Buffer.from([ slot, (address >> 8), (address & 0xFF), length ]) );
  }

  unwatch( slot ) {
    return this.command( ADDR_DONGLE, OP_UNWATCH, Buffer.from([ slot ]));

  }

  unwatchAll( slot ) {
    return this.command( ADDR_DONGLE, OP_UNWATCH_ALL );
